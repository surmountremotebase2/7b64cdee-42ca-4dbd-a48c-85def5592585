...
class TradingStrategy(Strategy):
   ...
   def run(self, data):
      ...
      close = [i["VIRT"]["close"] for i in data["ohlcv"]]
      ...
      try:
         if smavols2[-1]/smavols[-1]-1>0:
            if data["holdings"].get("VIRT", 0)>0 and close[-1]>close[-2]: 
               return TargetAllocation({"VIRT": data["holdings"].get("VIRT", 0)*0.93})
            else: out = smavols2[-1]/smavols[-1]-1
         else: out = 0
      except: return None
